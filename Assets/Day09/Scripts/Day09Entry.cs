...
{
    /// <summary>
    /// MonoBehaviour entry point for Day 09: The Graph Visualizer.
    /// Demonstrates setting up and using the GBG PlayableGraph Monitor to visualize
    /// PlayableGraph structure, node connections, and runtime state.
    /// 
    /// Follows the Playable Protocol:
    /// - Layer A: Data (Day09VisualizerData, Day08NamedGraphData, Day07ReverseData, Day06PlayStateData, Day05SpeedData, Day04RotatorData, Day02OutputHandle, Day01GraphHandle)
    /// - Layer B: Operations (VisualizerOps, GraphNamingOps, ReverseTimeOps, PlayStateOps, SpeedOps, RotatorLogic, ScriptPlayableOps)
    /// - Layer C: Extensions (Day09VisualizerExtensions, Day08NamedGraphExtensions, Day07ReverseExtensions, Day06PlayStateExtensions, Day05SpeedExtensions, Day04RotatorExtensions, Day02OutputHandleExtensions, Day01GraphHandleExtensions)
    /// </summary>
    public class Day09Entry : MonoBehaviour
    {
        // LAYER A: Data
        [SerializeField]
        ...
        // Visualizer settings
        [Header("Graph Visualizer Settings")]
        [SerializeField]
        private bool enableVisualizer = true;

        [SerializeField]
        private VisualizationDetailLevel detailLevel = VisualizationDetailLevel.Basic;

        [SerializeField]
        private VisualizerColorScheme colorScheme = VisualizerColorScheme.Default;
        ...
        [SerializeField]
        private string customGraphName = "VisualizedGraph";
        ...
        [SerializeField]
        private Color visualizerColor = Color.cyan;
        ...
        /// <summary>
        /// GUI controls for visualizer and graph manipulation.
        /// </summary>
        private void OnGUI()
        {
            ...
            if (visualizerData.IsValidVisualizer())
            {
                string visualizerText = $"Visualizer: {(visualizerData.IsActive ? "ON" : "OFF")}";
                GUI.Label(new Rect(startX, currentY, buttonWidth * 2, buttonHeight), visualizerText);
                currentY += buttonHeight + padding;

                string detailText = $"Detail: {visualizerData.DetailLevel}";
                GUI.Label(new Rect(startX, currentY, buttonWidth * 2, buttonHeight), detailText);
                currentY += buttonHeight + padding;

                string refreshText = $"Refresh: {visualizerData.RefreshInterval:F2}s";
                GUI.Label(new Rect(startX, currentY, buttonWidth * 2, buttonHeight), refreshText);
                currentY += buttonHeight + padding;
        ...
        private void InitializeGraphNaming()
        {
            ...
            {
                Debug.LogError("[Day 09] Cannot initialize graph naming: Graph is not initialized.");
                return;
            }

            if (!enableGraphNaming)
            {
                Debug.Log("[Day 09] Graph naming is disabled.");
                return;
            }
            ...
            Debug.Log("[Day 09] Graph naming initialized.");
        }
        ...
        private void InitializeVisualizer()
        {
            if (!graphHandle.IsActive)
            {
                Debug.LogError("[Day 09] Cannot initialize visualizer: Graph is not initialized.");
                return;
            }

            if (!enableVisualizer)
            {
                Debug.Log("[Day 09] Visualizer is disabled.");
                return;
            }

            PlayableGraph graph = graphHandle.Graph;
            var visualizer = visualizerData;
            visualizer.Initialize(in graph, detailLevel, colorScheme);
            visualizerData = visualizer;
            ...
            if (!showPortDetails)
            {
                visualizerData.ShowPortDetails = false;
            }

            if (!showConnections)
            {
                visualizerData.ShowConnections = false;
            }

            Debug.Log("[Day 09] Graph Visualizer initialized.");
        }
        ...
        private void InitializeOutput()
        {
            if (!graphHandle.IsActive)
            {
                Debug.LogError("[Day 09] Cannot initialize output: Graph is not initialized.");
                return;
            }
        ...
        private void InitializeRotator()
        {
            if (!graphHandle.IsActive)
            {
                Debug.LogError("[Day 09] Cannot initialize rotator: Graph is not initialized.");
                return;
            }
        ...
        private void InitializeSpeedControl()
        {
            if (!graphHandle.IsActive)
            {
                Debug.LogError("[Day 09] Cannot initialize speed control: Graph is not initialized.");
                return;
            }
        ...
        private void InitializeReverseTimeControl()
        {
            if (!graphHandle.IsActive)
            {
                Debug.LogError("[Day 09] Cannot initialize reverse time control: Graph is not initialized.");
                return;
            }

            if (!speedData.IsValidSpeedControl())
            {
                Debug.LogError("[Day 09] Cannot initialize reverse time control: Speed control is not initialized.");
                return;
            }
            ...
            Debug.Log("[Day 09] Reverse time control initialized.");
        }
        ...
        private void InitializePlayStateControl()
        {
            if (!graphHandle.IsActive)
            {
                Debug.LogError("[Day 09] Cannot initialize PlayState control: Graph is not initialized.");
                return;
            }
            ...
            Debug.Log("[Day 09] PlayState control initialized.");
        }
        ...
        private void LinkNodesTogether()
        {
            if (!speedData.IsValidSpeedControl())
            {
                Debug.LogError("[Day 09] Cannot link: Speed control is not valid.");
                return;
            }

            if (!rotatorData.IsValidRotator())
            {
                Debug.LogError("[Day 09] Cannot link: Rotator is not valid.");
                return;
            }
            ...
            Debug.Log("[Day 09] Successfully linked nodes: Output -> SpeedControl -> Rotator!");
        }

        private void LogSystemInfo()
        {
            Debug.Log("[Day 09] === System Information ===");
            
            if (visualizerData.IsValidVisualizer())
            {
                visualizerData.LogVisualizerInfo("Graph Visualizer");
            }
            ...
            Debug.Log("[Day 09] === End System Information ===");
        }
        ...
        private void UpdateVisualization()
        {
            ...
            else if (visualizerData.IsValidVisualizer())
            {
                targetColor = visualizerColor;
            }
            else
            {
            ...
            // Pulse effect based on speed and visualizer state
            if (speedData.IsValidSpeedControl())
            {
                float currentSpeed = speedData.GetCurrentSpeed();
        ...
        private void Reset()
        {
            ...
            detailLevel = VisualizationDetailLevel.Basic;
            colorScheme = VisualizerColorScheme.Default;
            ...
            customGraphName = "VisualizedGraph";
            ...
            visualizerColor = Color.cyan;
        }

#if UNITY_EDITOR
        /// <summary>
        /// Editor-only: Draw gizmos to visualize the graph state and visualizer info.
        /// </summary>
        private void OnDrawGizmos()
        {
            ...
            // Draw visualizer indicator (Day 09)
            if (visualizerData.IsValidVisualizer())
            {
                Gizmos.color = visualizerColor;
                Vector3 visualizerLabelPos = transform.position + Vector3.up * 4.5f;
                string config = visualizerData.GetConfigurationString();
                UnityEditor.Handles.Label(visualizerLabelPos, $"[VISUALIZER] {config}");
                
                // Draw visualizer status indicator
                Gizmos.color = visualizerData.IsActive ? visualizerColor : Color.gray;
